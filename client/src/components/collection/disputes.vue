<template>
  <div class="inner-stage">
    ...
    >
      ...
      <div slot="dispute_end_date" slot-scope="{row}">{{formatDate(row.dispute_end_date)}}</div>
      ...
      <div
        ...
        slot-scope="{row}"
      >
        <i v-if="tab == 1" style="color:#47c3ad" class="fa fa-user-circle"></i>
        <div
          class="exp"
          v-if="tab == 0"
          :class="[{'_good':  40 < getDaysLeft(row.dispute_end_date)}, {'_okay': 40 >= getDaysLeft(row.dispute_end_date) && getDaysLeft(row.dispute_end_date)  > 10}, {'_bad': 0 < getDaysLeft(row.dispute_end_date) && getDaysLeft(row.dispute_end_date) <= 10}, {'_expired': 0 >= getDaysLeft(row.dispute_end_date)}]"
        >
          <span
            v-if="0 < getDaysLeft(row.dispute_end_date)"
          >{{getDaysLeft(row.dispute_end_date)}} Days</span>
          <span v-if="0 >= getDaysLeft(row.dispute_end_date)">Expired</span>
        </div>
        <div class="clickName" style="margin-left:10px; text-transform:capitalize">{{row.first_name + ' ' + row.middle_name + ' ' + row.last_name}}</div>
      </div>
    ...
    <div style="display:flex; align-items:center;">
      ...
      <div>Page: {{page}}</div>
      <div @click="nextPage" v-if="total > 25" class="next-page">Next Page</div>
    </div>
  </div>
</template>
<script>
