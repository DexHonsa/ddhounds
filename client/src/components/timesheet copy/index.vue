              ...
              <div class="start_times">
                <div class="time-input-container">
                  <div class="time-input">
                    ...
                    <div @click="state[i].pickerOpen = !state[i].pickerOpen" class="time-input-time" :class="{'disabled':retrieved}">{{state[i].start != 0 ? startTimeOptions[state[i].start].formatted:null}}</div>
                  </div>
                  <v-expand-transition>
                  <div v-if="state[i].pickerOpen" class="time-picker">
                    <div v-for="(time, p) in startTimeOptions" @click="state[i].start = p, state[i].pickerOpen = false" :key="p" class="time-option">{{time.formatted}}</div>
                  </div>
                  </v-expand-transition>
                ...
                <div class="time-input-container">
                <div class="time-input">
                  <div class="time-input-title">Lunch In:</div>
                  <div @click="state[i].pickerOpen3 = !state[i].pickerOpen3" class="time-input-time" :class="{'disabled':retrieved}">{{state[i].l_start != 0 ? lunchTimeOptions[state[i].l_start].formatted :null}}</div>
                </div>
                <v-expand-transition>
                  <div v-if="state[i].pickerOpen3" class="time-picker">
                    <div v-for="(time, p) in lunchTimeOptions" @click="state[i].l_start = p, state[i].pickerOpen3 = false" :key="p" class="time-option">{{time.formatted}}</div>
                  </div>
                  </v-expand-transition>
                </div>
                ...
                <div class="time-input-container">
                <div class="time-input">
                  <div class="time-input-title">Lunch Out:</div>
                  <div @click="state[i].pickerOpen4 = !state[i].pickerOpen4" class="time-input-time" :class="{'disabled':retrieved}">{{state[i].l_end != 0 ? lunchTimeOptions[state[i].l_end].formatted : null}}</div>
                </div>
                <v-expand-transition>
                  <div v-if="state[i].pickerOpen4" class="time-picker">
                    <div v-for="(time, p) in lunchTimeOptions" @click="state[i].l_end = p, state[i].pickerOpen4 = false" :key="p" class="time-option">{{time.formatted}}</div>
                  </div>
                  </v-expand-transition>
                </div>
                ...
                <div class="time-input">
                  ...
                  <div @click="state[i].pickerOpen2 = !state[i].pickerOpen2" class="time-input-time" :class="{'disabled':retrieved}">{{ state[i].end != 0 ?  endTimeOptions[state[i].end].formatted : null}}</div>
                  
                </div>
                <v-expand-transition>
                  <div v-if="state[i].pickerOpen2" class="time-picker">
                    <div v-for="(time, p) in endTimeOptions" @click="state[i].end = p, state[i].pickerOpen2 = false" :key="p" class="time-option">{{time.formatted}}</div>
                  </div>
                  </v-expand-transition>
                </div>
                <div v-if="state[i].start != 0 && state[i].end != 0 && state[i].l_end != 0 && state[i].l_start != 0" class="total-hours">Hours: <span>{{getHours(i)}}</span> </div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    getHours(i){
      ...
      startTime.setHours(this.startTimeOptions[this.state[i].start].hours)
      startTime.setMinutes(this.startTimeOptions[this.state[i].start].minutes)

      let endTime = new Date();
      endTime.setHours(this.endTimeOptions[this.state[i].end].hours)
      endTime.setMinutes(this.endTimeOptions[this.state[i].end].minutes)

      let lstartTime = new Date();
      lstartTime.setHours(this.lunchTimeOptions[this.state[i].l_start].hours)
      lstartTime.setMinutes(this.lunchTimeOptions[this.state[i].l_start].minutes)

      let lendTime = new Date();

      lendTime.setHours(this.lunchTimeOptions[this.state[i].l_end].hours)
      lendTime.setMinutes(this.lunchTimeOptions[this.state[i].l_end].minutes)
  ...
  computed: {
    totalWeekHours(){
      ...
      for(let i = 0; i < this.state.length; i++){
        if(this.state[i].start != 0 && this.state[i].end != 0 && this.state[i].l_start != 0 && this.state[i].l_end != 0  ){
          let hours = this.getHours(i);
          hours = hours.split(':');
